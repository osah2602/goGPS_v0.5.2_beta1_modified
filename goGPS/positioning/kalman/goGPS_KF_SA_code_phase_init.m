...
%%COMPUTE A PRIORI ZENITH TROPOSPHERIC DELAYs(ZHD,ZWD)
if (flag_tropo)
%     [week, sow] = time2weektow(time_rx + zero_time);
%     date = gps2date(week, sow)
%     [~, mjd] = date2jd(date);
    
    %GET UTC/GREGORIAN DATE/TIME
    [week, sow] = time2weektow(time_rx + zero_time);
    UTCtime = gps2date(week, sow);
        
   %CONVERT CARTESIAN COORDs(XYZ) TO GEOGRAPHIC COORDs
   [phi_R, lam_R, h_R] = cart2geod(XR(1), XR(2), XR(3));
    
    %COMPUTE A PRIORI ZENITH HYDROSTATIC DELAY(ZHD)
    [~,~,~,~,apriori_ZHD,ZWD] = tropo_error_correction(UTCtime,[phi_R, lam_R, h_R],[]) ;
