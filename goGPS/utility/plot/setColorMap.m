function setColorMap(limitsC, perc, thrL, thrH)
if nargin < 2
    perc = 0.8;
end
if nargin < 3
...
cmapCenter = jet(round(posDiff./perc));
...
cmapLow = jet(round((posMin)/((1-perc)/2)));
cmapLow = cmapLow(1:posMin-1,:); 
cmapHigh = jet(round((length(caxVal)-posMax)/((1-perc)/2)));
