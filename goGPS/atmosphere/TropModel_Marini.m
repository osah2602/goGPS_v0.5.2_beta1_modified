...
%***************************************************************************
%DESCRIPTION:                                                              * 
%            "TropModel_Marini" is a subroutine that Computes the          *
%            Total Tropospheric Delays Using  Marini & Murray Model        *
%            Marini,J. W., & Murray Jr, C. W.(1973),modified Saastamoinen's*
%            model to compute the tropospheric range error using a continued 
%            partial fraction form / expansion.                            *
%=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
%USAGE:                                                                    *
%      General:                                                            *
%              [STD] = TropModel_Marini(ReceiverPos,SatPos,Temperature,... *
%                                                  Pressure,WaterVaporPres)*
%      Others:                                                             *
%I.         [STD]=TropModel_Marini(ReceiverPos,SatPos, [T  P  e])          *
%II.        [STD]=TropModel_Marini(ReceiverPos,SatPos,Temperature,...      *
%                                                              Pressure,[])*
%III.       [STD]=TropModel_Marini(ReceiverPos,SatPos,Temperature,[],[])   *
%IV.        [STD]=TropModel_Marini(ReceiverPos,SatPos,[],[],[])            * 
%V.         [STD]=TropModel_Marini(ReceiverPos,SatPos)                     *                     
%VI.        [STD]=TropModel_Marini(height,satElevation,Temperature,..      *
%                                                  Pressure,WaterVaporPres)*
%VII.       [STD]=TropModel_Marini(height,satElevation,[],[],[])           *
%VIII.      [STD]=TropModel_Marini(height,satElevation)                    *
%IX.        [STD]=TropModel_Marini([],satElevation)                        *
%X.         [STD]=TropModel_Marini([],satElevation, Temperature,...        *
%                                                  Pressure,WaterVaporPres)*
%XI.        [STD]=TropModel_Marini([],[],Temperature,Pressure,...          *
%                                                          WaterVaporPres) *
%=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
...
%==========================================================================
%REFERENCE:                                                                *
%1.        Marini, J. W., & Murray Jr, C. W. (1973). Correction of laser   *
%          range tracking data for atmospheric refraction at elevations    *
%          above 10 degrees.                                               *
%2.        Goad,C.C.& Goodman, L. (1974) A Modified Tropospheric Refraction* 
...
%3.        Farah, A. M. A. (2004). GPS/Galileo simulation for reduced dynamic 
%          LEO satellite orbit determination (Doctoral dissertation,...    *
...
function [STD,ZTD,ZHD,ZWD] = TropModel_Marini(ReceiverPos,SatPos,Temperature,...
                                                   Pressure,WaterVaporPres)
switch nargin
           ...
           t2=roundn(SatPos,3);
             ...
             if  size(SatPos,2)==1 %if the number of columns in SatPos is 1                                                      
               if  all(t2(:,1))==0                       
                 ...
                 satEL_rad=satEL_deg.* pi / 180;%#ok<*NASGU> %Elevation Angles converted to radian
...
%=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
%************TROPOHERIC DELAY MODELING USING MARINI & MURRAY MODEL
%=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
...
%Use Saastamoinen Zenith Hydrostatic Delay model to compute ZHD

%*************COMPUTE ORTHOMETRIC HEIGHT(H)
%--------------------------------------------------------------------------  
%NOTE:
%     SAASTAMOINEN MODEL REQUIREs +VE ORTHOMETRIC HEIGHT
...
%**********COMPUTE TOTAL TROPOSPHERIC DELAY 
 if ~exist('satEL_deg','var') 
   ...
   if exist('Rpos','var')
      ...
      satEL_deg = repmat(90,length(latR),1);   
 else 
       satEL_deg = repmat(90,length(P),1);
   end
   
   %****COMPUTE ZENITH TOTAL DELAY(ZTD)
   %Call the "SlantTropDelay.m" function
   ZTD = TropDelay(T,P,es,satEL_deg);
      
   %****COMPUTE ZENITH WET DELAY(ZWD)
   ZWD = ZTD - ZHD;
   
   %ASSIGN ZEROs TO SLANT DELAYs
   STD = zeros(size(ZTD));
   
 else 
     %****COMPUTE ZENITH TOTAL DELAY(ZTD)
     satEL = repmat(90,length(P),1);%Set Elevation Angle to 90 for ZTD
     ZTD = TropDelay(T,P,es,satEL);
     
     %****COMPUTE ZENITH WET DELAY(ZWD)
     ZWD = ZTD - ZHD;
     
     %****COMPUTE SLANT DELAYs
     %Call the "SlantTropDelay.m" function
     STD = TropDelay(T,P,es,satEL_deg);
     
 end   
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%END OF TropModel_Marini.m 
%=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- 
%=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-

%A.SUBROUTINE TO COMPUTE SLANT TROPOSPHERIC DELAY (SATELLITE IN VIEW)
function STD = TropDelay(T,P,es,satELEV)

%DESCRIPTION:
%            "SlantTropDelay" Computes Tropospheric Delay by ...           * 
%             Marini & Murray model.                                       *
%******SYNTAX:
%             STD = SlantTropDelay(T,P,e,satELEV);                         *
%
%******INPUT:
...
STD   = zeros(size(satELEV,1),size(satELEV,2));%Assign zeros of nxm to SHD
satEL = deal(STD);

A = zeros(size(satELEV,2),1);%A to conform with number of stations
[B,K] = deal(A);% Copy A content into B and K
...
for i=1:ncol %Loop over the Number of Receiver positions
   ...
   for j=1:nrow %Loop over the Number of Satellite Elevations
      ...
      %3.2.1.CONVERT ELEVATION ANGLEs TO RADIAN  
      %=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
      ...
      try %Goad & Goodman(1974)
          
         A(i,1) = 0.002277 .*(P(i,1) + ((1255 ./ T(i,1)) + 0.05).* es(i,1));        
         B(i,1) = 2.644e-3.*exp(-0.14372.*horth(i,1).*1.0e-3);         
      
      catch %Algorithm by Marini & Murray(1973)
           
           K(i,1) = 1.163-(0.00968.*cos(2.*latR(i,1)))-(0.00104.*T(i,1))+(0.00001435.*P(i,1));
           A(i,1) = 0.002357.*P(i,1)+0.000141.* es(i,1) ;
           B(i,1) = ((1.084e-8).*P(i,1).*T(i,1).*K(i,1))+((4.734e-8).*(P(i,1).^2/T(i,1)).*(2./(3-(1./K(i,1)))));
                    
      end    

      %****FINALLY, COMPUTE SLANT TOTAL DELAY(STD)
      STD(j,i) = (1./dgref(i,1)).*((A(i,1)+B(i,1))./(sin(satEL(j,i))+(B(i,1)./((A(i,1)+B(i,1)).*(sin(satEL(j,i))+0.015)))));%[m]
