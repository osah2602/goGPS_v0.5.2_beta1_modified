...
%***************************************************************************
%DESCRIPTION:                                                              * 
%            "TropModels" is a subroutine that Computes the wet,dry, and   * 
%            Total Tropospheric Delays Using  the various Tropopheric      *  
%            Delay Models                                                  *
%USAGE:                                                                    *
%     [STD,SHD,SWD,ZTD,ZHD,ZWD] = TropModels(Time,ReceiverPos,SatPos,dryModel,wetModel,...
                                             %MFh_model,MFw_model,metPARA,grid_dryModel,gridRES_dryModel,...
                                             %grid_wetModel,gridRES_wetModel,grid_MFh,gridRES_MFh,...
                                             %grid_MFw,gridRES_MFw,Timevar_dry,Timevar_wet,GTropCoeff,VMFgrids) 
%    OR

%[STD,SHD,SWD,ZTD,ZHD,ZWD] = TropModels(Time,ReceiverPos,ElevationAngle,dryModel,wetModel,...
%                                       MFh_model,MFw_model,metPARA,grid_dryModel,gridRES_dryModel,...
%                                       grid_wetModel,gridRES_wetModel,grid_MFh,gridRES_MFh,...
%                                       grid_MFw,gridRES_MFw,Timevar_dry,Timevar_wet,GTropCoeff,VMFgrids) 
%=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
...
%6.   MFh_model : Hydrostatic mapping function model type e.g.'Herring'    *
%7.   MFw_model : Wet mapping function model type e.g.'Chao'               *
%--------------------------------------------------------------------------*
%*.    METEOROLOGICAL PARAMETER:                                           *
%      =-=-=-=-=-=-=-=-=-=-=-=-=                                           *
%8     metPARA contains the following as in the order:                     *
%8.1  Temperature(T): Atmospheric Temperature in Degree Celsius(C)         *
%8.2    Pressure(P) : Atmospheric Pressure in millibars(mbar /hPa)         *
%8.3              e : Water vapor pressure in [mbar / hpa]                 *
%OTHERS (Optional, more specifically for 'Askne & Nordius' wet model) may  *                                     *
%       also include:                                                      *
%8.4.            Tm : Mean temperature of water vapor in kelvin(k)         *
%8.5.        lambda : water vapour `lapse rate'(dimensionless)]            * 
...
%=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-                *
%9. grid_dryModel : grid values in cells extracted from the grid file for  *
%                   HYDROSTATATIC DELAY MODEL                              *
%10.gridRES_dryModel: grid resolution (°) (possible: 1 or 5) for DRY model.*
%                     indicate 1 for 1°x1° external grid file and 5        *
%                     for 5°x5° external grid                              *
%11. grid_wetModel : grid values in cells extracted from the grid file for *
%                    WET DELAY MODEL                                       *
%12.gridRES_wetModel : grid resolution (°) (possible: 1 or 5) for the WET  * 
%                      model.indicate 1 for 1°x1° external grid file and 5 *
%                      for 5°x5° external grid                             *
%--------------------------------------------------------------------------*
%* GRID VALUES/FILES FOR GRID MF MODELS(GPT2w,GPT3)*                       *
%=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-                *
%13.grid_MFh : grid values in cells extracted from the grid file for       *
%              HYDROSTATATIC MAPPING FUNCTION(MF) MODEL                    *
%14.gridRES_MFh: grid resolution (°) (possible: 1 or 5) for HYD.MF model.  * 

%15.grid_MFw : grid values in cells extracted from the grid file for       *
%              WET MAPPING FUNCTION(MF) MODEL                              *
%16.gridRES_MFw: grid resolution (°) (possible: 1 or 5) for WET MF model.  * 
%17.Timevar_dry:0 i.e. 0 means GPT model with time variation, 1 means static    
%18.Timevar_wet:0 i.e. 0 means GPT model with time variation, 1 means static 
%19.GTrop_coeff : GTrop model coefficients / grid values extracted from ...*  
...
%20.VMFgrids     : Structure array of VMF grid files which can be :        *
%--------------------------------------------------------------------------*
...
%      grid_dryModel = []; gridRES_dryModel = []                           *
%    ----------------------------------------------                        *
%NB: SAME LOGIC APPLIES TO MAPPING FUNCTION MODELS                         *
%    ----------------------------------------------                        *
%=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-*
...
%        MFh_model and MFw_model can also be set to empty([]) as well      *
...
%        model type(tropo or MF model),then it should be entered as [].    *
%--------------------------------------------------------------------------+
...
%**************************************************************************+
%**************************************************************************+
function [STD,SHD,SWD,ZTD,ZHD,ZWD] = TropModels(Time,ReceiverPos,SatPos,dryModel,wetModel,...
                                                MFh_model,MFw_model,metPARA,grid_dryModel,gridRES_dryModel,...
                                                grid_wetModel,gridRES_wetModel,grid_MFh,gridRES_MFh,...
                                                grid_MFw,gridRES_MFw,Timevar_dry,Timevar_wet,GTropCoeff,VMFgrids)                   
...
switch nargin
    ...
    case {20,19,18,8} %Various inputs format
        ...
        if isempty(Time)
          ...
          %(1.2)*********CHECK HYDROSTATIC & WET MAPPING FUNCTION MODELS
          %*****HYDROSTATIC
          if ~isempty(MFh_model)
             ...
             if any([strncmpi(MFh_model,'VMF1(1° x 1°)',12),strncmpi(MFh_model,'VMF1(5° x 5°)',12),...
                     ...
                     strncmpi(MFh_model,'GMF',3),strncmpi(MFh_model,'NMF',3)])
                  
             %ISSUE ERROR MESSAGE  
             beep%Give a beep sound 
             errmsg000{1}=sprintf('Observation / Reception Time is not provided i.e it''s Empty ([ ]).\n');
             errmsg000{2}='Please provide Observation / reception Time & Try again.';
             errordlg(errmsg000,'Time Input Error','modal')  
          ...
          end %//if ~isempty(MFh_model)
          
          %***********WET
          if ~isempty(MFw_model)
             ...
             if any([strncmpi(MFw_model,'VMF1(1° x 1°)',12),strncmpi(MFw_model,'VMF1(5° x 5°)',12),...
                     ...
                     strncmpi(MFw_model,'GMF',3),strncmpi(MFw_model,'NMF',3)])
           
                %ISSUE ERROR MESSAGE  
                beep%Give a beep sound 
                errmsg9{1}=sprintf('Observation / Reception Time is not provided i.e it''s Empty ([ ]).\n');
                errmsg9{2}='Please provide Observation / reception Time & Try again.';
                errordlg(errmsg9,'Time Input Error','modal')  
      ...
      if isempty(ReceiverPos)
        ...
        %(2.2)*********CHECK HYDROSTATIC & WET MAPPING FUNCTION MODELS
        %HYDROSTATIC MF
        if ~isempty(MFh_model)
           ...
           if any([strncmpi(MFh_model,'VMF1(1° x 1°)',12),strncmpi(MFh_model,'VMF1(5° x 5°)',12),...
              ...
              %ISSUE ERROR MESSAGE  
              beep%Give a beep sound 
              errmsg12{1}=sprintf('Reciever / Site Position is not provided i.e it''s Empty([ ]).\n');
              errmsg12{2}='Please provide Receiver / Site position & Try again.';
              errordlg(errmsg12,'Reciever Position(s) Input Error','modal')  
        ...
        if ~isempty(MFw_model)
           ...
           if any([strncmpi(MFw_model,'VMF1(1° x 1°)',12),strncmpi(MFw_model,'VMF1(5° x 5°)',12),...
              ...
              %ISSUE ERROR MESSAGE  
              beep%Give a beep sound 
              errmsg13{1}=sprintf('Reciever / Site Position is not provided i.e it''s Empty([ ]).\n');
              errmsg13{2}='Please provide Receiver / Site position & Try again.';
              errordlg(errmsg13,'Reciever Position(s) Input Error','modal')  
           ...
           if size(ReceiverPos,2)==1 
              ...
              %*********CHECK HYDROSTATIC & WET MAPPING FUNCTION MODELS
              %***HYDROSTATIC MF
              if ~isempty(MFh_model)
                 ...
                 end  %//if any([strncmpi(MFh_model,'VMF1(1° x 1°)',12),...
      ...
      %(3)*****************CHECK SATELLITE POSITION / ELEVATION  INPUT
      %--------------------------------------------------------------------
      %SATELLITES POSITIONS / ELEVATIONS ARE REQUIRED TO COMPUTE SLANT
      %TROPOSPHERIC DELAYS(STD,SHD,SWD). IF SatPos INPUT IS EMPTY([]) AND
      %MAPPING FUNCTION INPUTS ARE ALSO EMPTY([]),IT IS ASSUME THAT USER 
      %WANTS TO COMPUTE ONLY ZENITH TROPOSPHERIC DELAYS
      %--------------------------------------------------------------------
      if isempty(SatPos)
          
         %(3.1)**CHECK HYDROSTATIC & WET MAPPING FUNCTION MODELS ARE NOT EMPTY([])
         if all([~isempty(MFh_model),~isempty(MFw_model)])
             
            %ISSUE ERROR MESSAGE  
            beep%Give a beep sound 
            errmsg14{1}=sprintf('Satellites Position(s) is not provided i.e it''s Empty([ ]).\n');
            errmsg14{2}='Please provide Satellite(s) Position(s) & Try again.';
            errordlg(errmsg14,'Satellite(s) Position(s) Input Error','modal')  
         ...
         elseif any([~isempty(MFh_model),~isempty(MFw_model)])
        
                %ISSUE ERROR MESSAGE  
                beep%Give a beep sound 
                errmsg15{1}=sprintf('Satellites Position(s) is not provided i.e it''s Empty([ ]).\n');
                errmsg15{2}='Please provide Satellite(s) Position(s) & Try again.';
                errordlg(errmsg15,'Satellite(s) Position(s) Input Error','modal') 
      ...
      end %//if isempty(SatPos)
      
      %(4)***************CHECK HYDROSTATIC TROPOPHERIC DELAY MODEL  INPUT
      %--------------------------------------------------------------------
      ...
      %(5)***************CHECK WET TROPOPHERIC DELAY MODEL  INPUT
      %--------------------------------------------------------------------
      ...
      %(6)**************CHECK HYDROSTATIC MAPPING FUNCTION(MF) MODEL  INPUT
      %--------------------------------------------------------------------
      %IF  HYDROSTATIC MAPPING FUNCTION MODEL IS EMPTY([]),ASSIGN DEFAULT MODEL      
      %--------------------------------------------------------------------
      if isempty(MFh_model)
          
        if ~isempty(SatPos)
          
         if any([strncmpi(MFw_model,'Saastamoinen',12),strncmpi(MFw_model,'Hopfield',8),strncmpi(MFw_model,'Moffett',7),...
                 strncmpi(MFw_model,'Ifadis',6),strncmpi(MFw_model,'Chao',4),strncmpi(MFw_model,'Herring',7),...
                 strncmpi(MFw_model,'Black',5),strncmpi(MFw_model,'Black & Eisner',14),strncmpi(MFw_model,'Simple',6),...
                 strncmpi(MFw_model,'GMF',3),strncmpi(MFw_model,'NMF',3),...
                 strncmpi(MFw_model,'VMF1(1° x 1°)',12),strncmpi(MFw_model,'VMF1(5° x 5°)',12),...
                 strncmpi(MFw_model,'VMF3(1° x 1°)',12),strncmpi(MFw_model,'VMF3(5° x 5°)',12)]) 
             
             
             %SET DEFAULT HYDROSTATIC MF MODEL USING WET MF MODEL
             MFh_model = MFw_model;
             
             if strncmpi(MFh_model,'NMF',3)
                 
                %PRINT SOME MESSAGE TO USER
                fprintf('Hydrostatic Mapping Function model is not provided i.e it''s Empty ([ ]).\n Using  Neill Mapping Function ( %s ) as default Hydrostatic Mapping Function model.',MFh_model) 
             
             elseif strncmpi(MFh_model,'GMF',3)
                 
                    %PRINT SOME MESSAGE TO USER
                    fprintf('Hydrostatic Mapping Function model is not provided i.e it''s Empty ([ ]).\n Using  Global Mapping Function ( %s ) as default Hydrostatic Mapping Function model.',MFh_model) 
             else
             
                 %PRINT SOME MESSAGE TO USER
                 fprintf('Hydrostatic Mapping Function model is not provided i.e it''s Empty ([ ]).\n Using  %s as default Hydrostatic Mapping Function model.',MFh_model)
                 
             end
             
         else
             %SET DEFAULT HYDROSTATIC MF MODEL 
             MFh_model ='NMF';
     ...
     %(7)**************CHECK WET MAPPING FUNCTION(MF) MODEL  INPUT
      %--------------------------------------------------------------------
      %IF  WET MAPPING FUNCTION MODEL IS EMPTY([]),ASSIGN DEFAULT MODEL      
      %--------------------------------------------------------------------
      if isempty(MFw_model)
          
        if ~isempty(SatPos)
          
         if any([strncmpi(MFh_model,'Saastamoinen',12),strncmpi(MFh_model,'Hopfield',8),strncmpi(MFh_model,'Moffett',7),...
                 strncmpi(MFh_model,'Ifadis',6),strncmpi(MFh_model,'Chao',4),strncmpi(MFh_model,'Herring',7),...
                 strncmpi(MFh_model,'Black',5),strncmpi(MFh_model,'Black & Eisner',14),strncmpi(MFh_model,'Simple',6),...
                 strncmpi(MFh_model,'GMF',3),strncmpi(MFh_model,'NMF',3),...
                 strncmpi(MFh_model,'VMF1(1° x 1°)',12),strncmpi(MFh_model,'VMF1(5° x 5°)',12),...
                 strncmpi(MFh_model,'VMF3(1° x 1°)',12),strncmpi(MFh_model,'VMF3(5° x 5°)',12)]) 
             ...
             if strncmpi(MFw_model,'NMF',3)
                 
                %PRINT SOME MESSAGE TO USER
                fprintf('Wet Mapping Function model is not provided i.e it''s Empty ([ ]).\n Using  Neill Mapping Function ( %s ) as default Wet Mapping Function model.',MFw_model) 
             
             elseif strncmpi(MFw_model,'GMF',3)
                 
                    %PRINT SOME MESSAGE TO USER
                    fprintf('Wet Mapping Function model is not provided i.e it''s Empty ([ ]).\n Using  Global Mapping Function ( %s ) as default Wet Mapping Function model.',MFw_model) 
             else
             
                 %PRINT SOME MESSAGE TO USER
                 fprintf('Wet Mapping Function model is not provided i.e it''s Empty ([ ]).\n Using  %s as default Wet Mapping Function model.',MFw_model)           
             end
      ...
      %(8)**************CHECK METEOROLOGICAL PARAMETERS  INPUT
      %--------------------------------------------------------------------
      if ~isempty(metPARA)
                ...
                if any([all([~any([strncmpi(dryModel,'GPT2 (5° x 5°)',14),strncmpi(dryModel,'GPT2w (1° x 1°)',14),strncmpi(dryModel,'GPT2w (5° x 5°)',14),strncmpi(dryModel,'VMF gridded ZHD',15),...
                     ...
                     if ~strncmpi(wetModel,'Askne & Nordius',15)
                        ...
                        return
                         ...
                         errmsg6{5}=sprintf('3. Mean Temperature of Water Vapor ( Tm ).\n');
                         errmsg6{6}=sprintf('4. Water Vapour Lapse Rate ( lambda ).\n');
     ...
     lambda = []; %Assign empty([]) matrix to lambda
      ...
      %=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-*
      %*****************WHEN ONLY 8 INPUTS ARE PROVIDED
      ...
      if nargin == 8 %FOR 8 INPUTS
         ...
         if ~isempty(wetModel)
          ...
          %********CHECK HYDROSTATIC & WET MAPPING FUNCTIONS MODEL TYPE
          %NOTE:IF MAPPING FUNCTION MODEL TYPE IS ONE OF THE VMF MODELS
          % WHICH REQUIRE GRID FILES & RESOLUTION.
          %----------------------------------------------------------------         
          ...
          if ~isempty(MFh_model)
             ...
             if any([strncmpi(MFh_model,'VMF1(1° x 1°)',12),strncmpi(MFh_model,'VMF1(5° x 5°)',12),...
                ...
                %ISSUE ERROR MESSAGE  
                beep%Give a beep sound 
                errmsg18{1}=sprintf('No input for VMF / GPT grid file.\n');
                errmsg18{2}='Please provide VMF / GPT grid file & Try again.';
                errordlg(errmsg18,'VMF / GPT Input Error','modal') 
          ...
          if ~isempty(MFw_model)
             ...
             if any([strncmpi(MFw_model,'VMF1(1° x 1°)',12),strncmpi(MFw_model,'VMF1(5° x 5°)',12),...
                ...
                %ISSUE ERROR MESSAGE  
                beep%Give a beep sound 
                errmsg19{1}=sprintf('No input for VMF / GPT grid file.\n');
                errmsg19{2}='Please provide VMF / GPT grid file & Try again.';
                errordlg(errmsg19,'VMF / GPT Input Error','modal') 
     ...
     %=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-*
      %*****************WHEN ALL 20 INPUTS ARE PROVIDED
     ...
     %****************CHECH IF ALL INPUTS ARE PROVIDED
      elseif any(nargin == [20,19]) %FOR 20 INPUTS
           ...
           %2.CHECK GRID FILE FOR WET TROPO MODELS
            ...
            %           ------------------------------------------------
            %***********CHECK VIENNA MAPPING FUNCTION(VMF) MODELS INPUTS
            %           ------------------------------------------------
            %NOTE:
            %     GENERALLY, VMF1 / VMF3 MODELS REQUIRE VMF1 / VMF3 grid files TO
            %     COMPUTE MAPPING FUNCTIONS.WHERE VMF GRID FILES ARE NOT FOUND 
            %     OR PROVIDED, GPT MODELS(I.E.GPT2,GPT2w,GPT3) GRID VALUES ARE USED 
            %     TO COMPUTE MAPPING FUNCTIONS.
            %--------------------------------------------------------------
            
            %3.CHECK GRID FILE FOR HYDROSTATIC MAPPING FUNCTION MODELS
            
            if all([any([all([isempty(grid_MFh),isempty(gridRES_MFh )]),...
                    any([isempty(grid_MFh),isempty(gridRES_MFh )])]),...
                    any([getappdata(0,'VMF_grid_found')==0,isempty(getappdata(0,'VMF_grid_found'))])])
               
               if ~isempty(MFh_model)
                   
                  %IF PROVIDED MODEL(s) IS ANY OF THE VMF HYD MODELS,ISSUE MSG
                  if any([strncmpi(MFh_model,'VMF1(1° x 1°)',12),strncmpi(MFh_model,'VMF1(5° x 5°)',12),...
                          strncmpi(MFh_model,'VMF3(1° x 1°)',12),strncmpi(MFh_model,'VMF3(5° x 5°)',12)])
                   
                     if isempty(grid_MFh)& isempty(gridRES_MFh )

                        if any([strncmpi(MFh_model,'VMF1(1° x 1°)',12),strncmpi(MFh_model,'VMF1(5° x 5°)',12)])  
                        
                           %ISSUE ERROR MESSAGE  
                           beep%Give a beep sound 
                           errmsg34{1}=sprintf('No input for GPT2 / GPT2w  grid  values & grid resolution ...\n');
                           errmsg34{2}=sprintf('to compute Hydrostatic Mapping Function . Please provide ... \n');
                           errmsg34{3}=sprintf('GPT2 / GPT2w  grid  values & grid resolution & Try again....\n');
                           errordlg(errmsg34,'GPT2 / GPT2w grid values Input Error','modal')
                           
                        elseif any([strncmpi(MFh_model,'VMF3(1° x 1°)',12),strncmpi(MFh_model,'VMF3(5° x 5°)',12)])  
                        
                               %ISSUE ERROR MESSAGE  
                               beep%Give a beep sound 
                               errmsg34_1{1}=sprintf('No input for GPT3  grid  values & grid resolution ...\n');
                               errmsg34_1{2}=sprintf('to compute Hydrostatic Mapping Function . Please provide ... \n');
                               errmsg34_1{3}=sprintf('GPT3  grid  values & grid resolution & Try again....\n');
                               errordlg(errmsg34_1,'GPT3 grid values Input Error','modal')
                     ...
                     elseif isempty(grid_MFh)& ~isempty(gridRES_MFh )
                              
                            if any([strncmpi(MFh_model,'VMF1(1° x 1°)',12),strncmpi(MFh_model,'VMF1(5° x 5°)',12)])  
                        
                               %ISSUE ERROR MESSAGE  
                               beep%Give a beep sound 
                               errmsg35{1}=sprintf('No input for GPT2 / GPT2w  grid  values to compute...\n');
                               errmsg35{2}=sprintf('Hydrostatic Mapping Function . Please provide ... \n');
                               errmsg35{3}=sprintf('GPT2 / GPT2w  grid  values & Try again....\n');
                               errordlg(errmsg35,'GPT2 / GPT2w grid values Input Error','modal')
                           
                            elseif any([strncmpi(MFh_model,'VMF3(1° x 1°)',12),strncmpi(MFh_model,'VMF3(5° x 5°)',12)])  
                        
                                   %ISSUE ERROR MESSAGE  
                                   beep%Give a beep sound 
                                   errmsg35_1{1}=sprintf('No input for GPT3  grid  values to compute ...\n');
                                   errmsg35_1{2}=sprintf('Hydrostatic Mapping Function . Please provide ... \n');
                                   errmsg35_1{3}=sprintf('GPT3  grid  values & Try again....\n');
                                   errordlg(errmsg35_1,'GPT3 grid values Input Error','modal')
                     ...
                     elseif ~isempty(grid_MFh)& isempty(gridRES_MFh)
                            ...
                            %ISSUE ERROR MESSAGE  
                            beep%Give a beep sound 
                            errmsg36{1}=sprintf('No input for GPT grid resolution to compute Hydrostatic ...\n');
                            errmsg36{2}=sprintf('Mapping Function, i.e. input is empty ( [ ] ).\n');
                            errmsg36{3}='Please provide GPT grid resolution & Try again.';
                            errordlg(errmsg36,'GPT grid resolution Input Error','modal') 
                     ...
                     end %//if isempty(grid_MFh)& isempty(gridRES_MFh)
                  
                  end  %//if any([strncmpi(MFh_model,'VMF1(1° x 1°)',12),...
                  
               end %//if ~isempty(MFh_model)
               
            end  %//if any([all([isempty(grid_MFh),isempty(gridRES_MFh)]),...
            
            
           %4.CHECK GRID FILE FOR WET MAPPING FUNCTION MODELS
            if all([any([all([isempty(grid_MFw),isempty(gridRES_MFw )]),...
                    any([isempty(grid_MFw),isempty(gridRES_MFw )])]),...
                    any([getappdata(0,'VMF_grid_found')==0,isempty(getappdata(0,'VMF_grid_found'))])])
                
              if ~isempty(MFw_model)
               
               %IF PROVIDED MODEL(s) IS ANY OF THE VMF WET MODELS,ISSUE MSG
               if any([strncmpi(MFw_model,'VMF1(1° x 1°)',12),strncmpi(MFw_model,'VMF1(5° x 5°)',12),...
                       strncmpi(MFw_model,'VMF3(1° x 1°)',12),strncmpi(MFw_model,'VMF3(5° x 5°)',12)])
                   
                  if isempty(grid_MFw)& isempty(gridRES_MFw )
                      
                     if any([strncmpi(MFw_model,'VMF1(1° x 1°)',12),strncmpi(MFw_model,'VMF1(5° x 5°)',12)])  
                        
                        %ISSUE ERROR MESSAGE  
                        beep%Give a beep sound 
                        errmsg37{1}=sprintf('No input for GPT2 / GPT2w  grid  values & grid resolution ...\n');
                        errmsg37{2}=sprintf('to compute Wet Mapping Function . Please provide ... \n');
                        errmsg37{3}=sprintf('GPT2 / GPT2w  grid  values & grid resolution & Try again....\n');
                        errordlg(errmsg37,'GPT2 / GPT2w grid values Input Error','modal')
                           
                     elseif any([strncmpi(MFw_model,'VMF3(1° x 1°)',12),strncmpi(MFw_model,'VMF3(5° x 5°)',12)])  
                        
                            %ISSUE ERROR MESSAGE  
                            beep%Give a beep sound 
                            errmsg37_1{1}=sprintf('No input for GPT3  grid  values & grid resolution ...\n');
                            errmsg37_1{2}=sprintf('to compute Wet Mapping Function . Please provide ... \n');
                            errmsg37_1{3}=sprintf('GPT3  grid  values & grid resolution & Try again....\n');
                            errordlg(errmsg37_1,'GPT3 grid values Input Error','modal')
                  ...
                  elseif isempty(grid_MFw)& ~isempty(gridRES_MFw)
                              
                         if any([strncmpi(MFw_model,'VMF1(1° x 1°)',12),strncmpi(MFw_model,'VMF1(5° x 5°)',12)])  
                        
                               %ISSUE ERROR MESSAGE  
                               beep%Give a beep sound 
                               errmsg38{1}=sprintf('No input for GPT2 / GPT2w  grid  values to compute...\n');
                               errmsg38{2}=sprintf('Wet Mapping Function . Please provide ... \n');
                               errmsg38{3}=sprintf('GPT2 / GPT2w  grid  values & Try again....\n');
                               errordlg(errmsg38,'GPT2 / GPT2w grid values Input Error','modal')
                           
                         elseif any([strncmpi(MFw_model,'VMF3(1° x 1°)',12),strncmpi(MFw_model,'VMF3(5° x 5°)',12)])  
                        
                                %ISSUE ERROR MESSAGE  
                                beep%Give a beep sound 
                                errmsg38_1{1}=sprintf('No input for GPT3  grid  values to compute ...\n');
                                errmsg38_1{2}=sprintf('Wet Mapping Function . Please provide ... \n');
                                errmsg38_1{3}=sprintf('GPT3  grid  values & Try again....\n');
                                errordlg(errmsg38_1,'GPT3 grid values Input Error','modal')
                  ...
                  elseif ~isempty(grid_MFw)& isempty(gridRES_MFw)
                         ...
                         %ISSUE ERROR MESSAGE  
                         beep%Give a beep sound 
                         errmsg39{1}=sprintf('No input for GPT grid resolution to compute Wet ...\n');
                         errmsg39{2}=sprintf('Mapping Function, i.e. input is empty ( [ ] ).\n');
                         errmsg39{3}='Please provide GPT grid resolution & Try again.';
                         errordlg(errmsg39,'GPT grid resolution Input Error','modal') 
                  ...
                  end %//if isempty(grid_MFw)& isempty(gridRES_MFw)
                  
               end %//if any([strncmpi(MFw_model,'VMF1(1° x 1°)',12),...
               
              end %//if ~isempty(MFh_model)
               
            end %//if any([all([isempty(grid_MFw),isempty(gridRES_MFw)]),...
...
[ZTD, ZHD, ZWD] =ZenithTropDelay(UTCtime,T,P,es,Tm,lambda,dryModel,wetModel,...
                                 grid_dry,gridRES_dry,grid_wet,gridRES_wet,Timevar_dry,Timevar_wet,GTropCoeff,VMFgrids) ;   
...
if any([~exist('satpos_empty','var'),exist('satEL_deg','var')])
   ...
   [MFh, MFw] = MF(UTCtime,latD,lonD,h,satEL_deg,MFh_model,MFw_model,T,P,es,...
                   grid_MFh,gridRES_MFh,grid_MFw,gridRES_MFw,Timevar,VMFgrids);                         
...
%SAVE DRY & WET TROPO MODELS TO BE CALLED AT MF sub-routine
...
elseif any([strncmpi(dryModel,'VMF gridded ZHD',15),strfind(dryModel,'VMF gridded ZHD')])
       ...
       %Call the "SearchReadVMFgrid.m" function(External) 
