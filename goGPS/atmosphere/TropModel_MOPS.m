...
%***************************************************************************
%DESCRIPTION:                                                              * 
%            "TropModel_MOPS" is a subroutine that Computes the wet, dry   *
%             and Total Tropospheric Delays Using RTCA- MOPS (Minimum...   *
%             Operational Performance Standards) Tropospheric Delay Model  * 
%             (RCTA, 2001). The RTCA- MOPS model provides an estimate of   * 
%             the zenith total tropospheric delay that is dependent on     *
%             empirical estimates of meteorological parameters such        *
%             as, pressure, temperature, water vapour pressure,            *
%             temperature lapse rate and water vapour lapse rate.This model* 
%             was designed for users without access to measurements of     *
%             these meteorological parameters at ground level. Input of    *
%             receiver latitude, day of year and height above the ellipsoid* 
%             are required to interpolate a table of values to get the     *
%             ground level weather parameters. By using an appropriate     *
%             elevation angle, dependent mapping function such as the Black*                         
%             and Eisner(1984) model,the Zenith delays  are then scaled to *
%             the slant or receiver-satellite line of sight(LOS) Direction.*
%USAGE:                                                                    *
%     General:                                                             *
%             [STD,SHD,SWD,ZTD,ZHD,ZWD] = TropModel_MOPS(Time,ReceiverPos, *
%                                                                   SatPos)*
%      Others:
%             [STD,SHD,SWD,ZTD,ZHD,ZWD] = TropModel_MOPS(Time,ReceiverPos, *
%                                                           ElevationAngle)*
...
%                                                            RefEllipsoid);*
%5.[STD,SHD,SWD,ZTD,ZHD,ZWD] = MOPS(UTCtime,lat,hgt,satEL);                *
%6.[AVG,AMP] =interpolate_MOPS(UserLat,TableLat,HydavgValues,...           *
%                                                                ampValues)*
%7.[T, P, E,TM,ZTD,ZHD,ZWD]=getMETpara_MOPS(intAVG,intAMP,UTCtime,lat,hgt) *
...
function [STD,SHD,SWD,ZTD,ZHD,ZWD] = TropModel_MOPS(Time,ReceiverPos,SatPos)
...
%=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
%************TROPOHERIC DELAY MODELING USING MOPS MODEL
%=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-          
%1.***************COMPUTE ZENITH TROPOSPHERIC DELAYs
%Call the "ZenithTropDelay_MOPS.m" function
[ZTD, ZHD, ZWD,T] = ZenithTropDelay_MOPS(UTCtime,latD,lonD,h);

%1.***************COMPUTE SLANT TROPOSPHERIC DELAYs
if any([exist('satEL_deg','var'),~exist('satpos_empty','var')])    
%Call the "SlantTropDelay_MOPS.m" function
...
setappdata(0,'T_MOPS',T)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% TropModel_MOPS.m
%=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
                                        
%A.******SUBROUTINE TO COMPUTE ZENITH TROPOSPHERIC DELAY
function [ZTD, ZHD, ZWD, T] = ZenithTropDelay_MOPS(UTCtime,lat,lon,hgt)
...
%            the RTCA- MOPS (Minimum Operational Performance Standards)... *
%            Latitude and Day of Year (DOY) dependent Tropospheric model.  *
%            The MOPS model uses the explicit forms of Saastamoinen's delay*
%            algorithms combined with  Black and Eisner (1984) mapping ... * 
%            function model.                                               *
%USAGE:                                                                    *
%      General:                                                            *
%              [ZTD,ZHD,ZWD] = ZenithTropDelay(UTCtime,lat,lon,hgt)        * 
%=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
%****INPUTs:                                                               *
...
k3 = 382000;%Refractivity Constant in [K/hPa]     
...
[intAVG,intAMP] =interpolate_MOPS(lat,AVG,AMP);
...
TM = T * (1 - (BETA * Rd / (gm*(LAMBDA+1)))); %#ok<*NASGU>
...
ZWD = ((1.0e-6 *k3* Rd )/((gm*(LAMBDA + 1))- BETA*Rd))* E/ T;
...
%B.SUBROUTINE TO INTERPOLATE MOPS AVEARAGE & AMPLITUDE VALUEs
function [AVG,AMP] =interpolate_MOPS(UserLat,avgValues,ampValues)
                                                                 
%***************************************************************************
%DESCRIPTION:                                                              * 
%            The function  "interpolate_MOPS" Interpolates and computes   * 
%            averaged and seasonal variation Meteorological parameters...  *   
%            used for tropospheric delay prediction, given receiver...     * 
%            latitude based on the MOPS Model.Parameters above |lat|<=15° *   
...
%          1       2     3       4       5                                 *
%     AVG:[P       T     E     beta   lambda]                              *
%     AMP:[dP      dT   dE    dbeta   lambda]                              *
%=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-+
%==========================================================================+
%  Written by: OSAH SAMUEL, Msc Geomatic Engineering ,2016                 *    
%       Email: osahsamuel@yahoo.ca                                         *
