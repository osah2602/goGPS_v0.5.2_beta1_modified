...
%***************************************************************************
%DESCRIPTION:                                                              * 
%*************"TropModel_GPT2" is a subroutine that determines the         *
%             Meteorological Parameters and then Computes the wet,dry, and *
%             Total Tropospheric Delays Using Hydrostatic Delay Model by   *  
%             Saatamoinen and Vienna Mapping function 1 (VMF1).            *              
%*************The subroutine determines pressure, temperature, temperature *
...
%             a 5 x 5 degree external grid file ('gpt2_5.grd') with mean   *
%             values as well as sine and cosine  amplitudes for  the       *
%             annualand semiannual variation of the coefficients.          * 
%=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-*
...
%USAGE:
%      [STD,SHD,SWD,ZTD,ZHD,ZWD] = TropModel_GPT2(Time,ReceiverPos,SatPos,grid,Timevar)
%                                                           
%Others:
%      [STD,SHD,SWD,ZTD,ZHD,ZWD] = TropModel_GPT2(Time,ReceiverPos,ElevationAngle,grid,Timevar)
%                                                   
%      [STD,SHD,SWD,ZTD,ZHD,ZWD] = TropModel_GPT2(Time,ReceiverPos,[],grid,Timevar)
%                                                         
%      [STD,SHD,SWD,ZTD,ZHD,ZWD] = TropModel_GPT2(Time,ReceiverPos,[],grid)
%                                                           
%*******EXAMPLE:                                                           +
%=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-+
...
%2. NOW RUN THE "TropModel_GPT2.m" TO COMPUTE ZENITH DELAYS                +
%INPUTS(eg):                                                               +
%1.Time        = [2019 1 5]                                                +
...
%5.grid_res    = 5                                                         +
%6.Timevar     = 0 i.e. 0 means GPT2 with time variation, 1 means static   +

%==> [STD,SHD,SWD,ZTD,ZHD,ZWD] = TropModel_GPT2(Time,ReceiverPos,SatPos,grid,grid_res,Timevar)
%
%i.e.[STD,SHD,SWD,ZTD,ZHD,ZWD] = TropModel_GPT2([2019 1 5],[6 40 3.7449 -1 34  0.7263  260.2410],[],grid,5,0)
%=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-+
...
%Original codes by Lagler et al 2013                                       *
...
function [STD,SHD,SWD,ZTD,ZHD,ZWD] = TropModel_GPT2(Time,ReceiverPos,SatPos,grid,Timevar)
...
switch nargin
    ...
    case {5,4,3,2} %When all inputs are provided        
        ...
        if (any(nargin==[5,4,3,2]))
          ...
          elseif  nargin ==2 %for ONLY three(3) inputs...   
...
%=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
%************TROPOHERIC DELAY MODELING USING USING GPT2 5x5 degree MODEL
%=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  
%1.********COMPUTE ZENITH TROPOSPHERIC DELAYs
%Call the "getZTD_GPT2.m" Function
[ZHD,ZWD,ZTD,ah,aw,T] = getZTD_GPT2(UTCtime,latR,lonR,h,grid,Timevar); %#ok<*ASGLU>
...
%             Saastamoinen Zenith Hydrostaic and Wet Tropospheric Delay    *
%             models and Vienna  Mapping function 1 (VMF1)for the Slant... *
%             delays.                                                      *            
%******SYNTAX:                                                             *
...
function [ZHD,ZWD,ZTD,ah,aw,T] = getZTD_GPT2(UTCtime,lat,lon,hell,grid,Timevar)  

%********COMPUTE ZTDs USING GPT2 MODEL
  ...
  if isequal(Identical,1) %if ROWS ARE IDENTICAL
     ...
     [ZWD,ZTD,ah,aw,T]=deal(ZHD);%create copy of ZHD in ZWD,ZTD,ah,aw,T
      ...
      for i=1:nrow_time
         ...
         [ZHD(i,1),ZWD(i,1),ZTD(i,1),ah(i,1),aw(i,1),T(i,1)] = GPT2_5_x_5(UTCtime(i,:),lat(i),lon(i),hell(i),grid,Timevar);
      end
      ...
      for i=1:nrow_time %LOOP OVER TIME
         ...
         for j=1:nrow_pos %LOOP OVER POSITIONS
           ...
           [ZHD(j,i),ZWD(j,i),ZTD(j,i),ah(j,i),aw(j,i),T(j,i)] = GPT2_5_x_5(UTCtime(i,:),lat(j),lon(j),hell(j),grid,Timevar);  
...
%***********************END OF getZTD_GPT2.m ***********************    
%=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-

%B_1.******SUBROUTINE TO COMPUTE METEOROLOGICAL PARAMETERS FOR ZTDs
function [ZHD,ZWD,ZTD,ah,aw,T] = GPT2_5_x_5(UTCtime,lat,lon,hell,grid,Timevar)
...
%             coefficients.                                                *
%USAGE:                                                                    *
%      [ZHD,ZWD,ZTD,ah,aw] = GPT2(UTCtime,lat,lon,hell,grid,Timevar)       *
%INPUTs:                                                                   *
...
ZWD =(0.002277.*((1255 ./ T) + 0.05).* e)./dgref;% Zenith wet delay[m]
...
%C_1.SUBROUTINE TO COMPUTE VIENNA MAPPING FUNCTION1(VMF1)
