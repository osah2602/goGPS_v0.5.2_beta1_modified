...
%***************************************************************************
%DESCRIPTION:                                                              * 
%            "TropModel_GG" is a subroutine that Computes the wet,dry      *
%             and Total Tropospheric Delays Using  Modified Hopfield Model *
%             by Goad,C.C.& Goodman, L. (1974).                            *

%            Goad,C.C. & Goodman, L.(1974),utilized the Hopfield quartic 
%            refractivity model and introduced lengths of position  vectors 
%            instead  of  heights.
%=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
%USAGE:                                                                    *
%      General:                                                            *
%             [STD,SHD,SWD,ZTD,ZHD,ZWD]=TropModel_GG(ReceiverPos,SatPos,...*
%                                      Temperature,Pressure,WaterVaporPres)*

%      Others:                                                             *
%I.         [STD,SHD,SWD,ZTD,ZHD,ZWD]=TropModel_GG(ReceiverPos,SatPos,...  *
%                                                               [T  P  e]) *
%II.        [STD,SHD,SWD,ZTD,ZHD,ZWD]=TropModel_GG(ReceiverPos,SatPos,...  *
%                                                  Temperature,Pressure,[])*
%III.       [STD,SHD,SWD,ZTD,ZHD,ZWD]=TropModel_GG(ReceiverPos,SatPos,...  *
%                                                   Temperature,[],[])     *
%IV.        [STD,SHD,SWD,ZTD,ZHD,ZWD]=TropModel_GG(ReceiverPos,SatPos,...  *
%                                                                [],[],[]) *   
%V.         [STD,SHD,SWD,ZTD,ZHD,ZWD]=TropModel_GG(ReceiverPos,SatPos)     *                      *
%VI.        [STD,SHD,SWD,ZTD,ZHD,ZWD]=TropModel_GG(height,satElevation,... *
%                                      Temperature,Pressure,WaterVaporPres)*
%VII.       [STD,SHD,SWD,ZTD,ZHD,ZWD]=TropModel_GG(height,satElevation,... *
%                                                                 [],[],[])*
%VIII.      [STD,SHD,SWD,ZTD,ZHD,ZWD]=TropModel_GG(height,satElevation)    *
%IX.        [STD,SHD,SWD,ZTD,ZHD,ZWD]=TropModel_GG([],satElevation)        *
%X.         [STD,SHD,SWD,ZTD,ZHD,ZWD]=TropModel_GG([],satElevation,...     *
%                                      Temperature,Pressure,WaterVaporPres)*
%XI.        [STD,SHD,SWD,ZTD,ZHD,ZWD]=TropModel_GG([],[],Temperature...    *
...
%                                                                          *
%5.        Goad,C.C.& Goodman, L. (1974) A Modified Tropospheric Refraction* 
...
%6         Goad,C.C.,& Goodman,L.(1974).A Modified Hopfield Tropospheric   *
%          Refraction Correction Model. Paper Presented at the Fall Annual 
%          Meeting of the American Geophysical Union, San Francisco, 
%          California, USA, December 12-17.

%7.        Modeling of Tropospheric Delays Using ANFIS,Wayan Suparta;...   *
%                                                Kemal Maulana Alhasa(2016)*
%8.        GPS Theory,Algorithms and Applications 2ed,Guochang Xu,June 2007*
...
function [STD,SHD,SWD,ZTD,ZHD,ZWD]=TropModel_GG(ReceiverPos,SatPos,...
                                           Temperature,Pressure,WaterVaporPres)
...
%=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
%TROPOHERIC DELAY MODELING USING MODIFIED HOPFIELD MODEL(Goad & Goodman(1974) 
%=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- 
...
%             Goad & Goodman (1974)-Modified Hopfield Model                *
...
%**COMPUTE DRY & WET TROPO REFRACTIVITY(N) @ MSL
...
try
   ZTD = ZHD + ZWD;%Zenith Total Delay [m]
     ...
     ZTD = (1.0e-6 / 5) .*((N_dry0 .* hDry)+(N_wet0 .* hWet));%Zenith  Total Delay[m]
end 
...
for i = 1:ncol %Loop over the Number of Receiver positions
   ...
   for j = 1:nrow %Loop over the Number of Satellite Elevations
      ...
      SHD(j,i) = 1.0e-6*N_dry0(i)*MFh(j,i) ;

      %COMPUTE Slant Wet Delay(ZWD)
      SWD(j,i) = 1.0e-6*N_wet0(i)*MFw(j,i);
...
%C.******SUBROUTINE TO COMPUTE TROPOSHERIC POLYNOMIALS
...
%3.******SUBROUTINE TO COMPUTE ELLIPSOID PARAMETERS
