...
%***************************************************************************
%DESCRIPTION:                                                              * 
%            "TropModel_Black" is a subroutine that Computes the wet,dry   *
%             and Total Tropospheric Delays Using  Black Model             *

%             Black,(1978) developed a model, which is a modification of   *
%             the Hopfield model.The model uses constants to define water  *  
%             vapour in the refractivity model depending on season and ... *
%             latitude.
%=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
%******USAGE:                                                              *
% General:                                                                 *
%         [STD,SHD,SWD,ZTD,ZHD,ZWD]=TropModel_Black(ReceiverPos,SatPos,... *
%                                      Temperature,Pressure,WaterVaporPres)*
%  Others:
%I.       [STD,SHD,SWD,ZTD,ZHD,ZWD]=TropModel_Black(ReceiverPos,SatPos,... *
%                                                                [T  P  e])*
%II.      [STD,SHD,SWD,ZTD,ZHD,ZWD]=TropModel_Black(ReceiverPos,SatPos,... *
%                                                  Temperature,Pressure,[])
%III.     [STD,SHD,SWD,ZTD,ZHD,ZWD]=TropModel_Black(ReceiverPos,SatPos,... *
%                                                   Temperature,[],[])     *
%IV.      [STD,SHD,SWD,ZTD,ZHD,ZWD]=TropModel_Black(ReceiverPos,SatPos,... *
%                                                                [],[],[]) *   
%V.       [STD,SHD,SWD,ZTD,ZHD,ZWD]=TropModel_Black(ReceiverPos,SatPos)    *                      *
%VI.      [STD,SHD,SWD,ZTD,ZHD,ZWD]=TropModel_Black(height,satElevation,...*
%                                      Temperature,Pressure,WaterVaporPres)*
%VII.     [STD,SHD,SWD,ZTD,ZHD,ZWD]=TropModel_Black(height,satElevation,...*
%                                                                 [],[],[])*
%VIII.    [STD,SHD,SWD,ZTD,ZHD,ZWD]=TropModel_Black(height,satElevation)   *
%IX.      [STD,SHD,SWD,ZTD,ZHD,ZWD]=TropModel_Black([],satElevation)       *
%X.       [STD,SHD,SWD,ZTD,ZHD,ZWD]=TropModel_Black([],satElevation,...    *
%                                      Temperature,Pressure,WaterVaporPres)*
%XI.      [STD,SHD,SWD,ZTD,ZHD,ZWD]=TropModel_Black([],[],Temperature...   *
...
%2.        Black, H. D., & Eisner, A. (1984). Correcting satellite Doppler *
...
function [STD,SHD,SWD,ZTD,ZHD,ZWD] = TropModel_Black(ReceiverPos,SatPos,Temperature,...
                                                   Pressure,WaterVaporPres)
...
%=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
%************TROPOHERIC DELAY MODELING USING BLACK MODEL
%=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
...
[STD, SHD, SWD] = SlantTropDelay(ZHD,ZWD,T,satEL_deg);
...
%             Black model.                                                 *
%******SYNTAX:
%             [ZTD, ZHD, ZWD] = ZenithTropDelay(T,P);                     *
%
%******INPUT:
...
latD = getappdata(0,'latD');%Get station latitude [degrees]
   
%*****COMPUTE ZENITH WET DELAY(ZWD)
try
   
   if ~isempty(latD) %Chech if latRAD is not empty
        
     %*****INITIALIZING ZWD OUTPUT VARIABLE
      ZWD = zeros(size(latD,1),1);%Assign zeros of nxm to ZWD
     ...
     %******CHECK WHICH REGION Latitude FALLs
      for i = 1:size(latD,1)
        ...
        end    %if abs(latD(i)) < 23.5
        
      end %\\for i=1:size(latD,1)
...
function [STD, SHD, SWD] = SlantTropDelay(ZHD,ZWD,T,satELEV)

%DESCRIPTION:
%            "SlantTropDelay" Computes Slant Tropospheric Delay by ...     * 
%             Black model.                                                 *
%******SYNTAX:
%             [STD, SHD, SWD] = SlantTropDelay(ZHD,ZWD,T,satELEV);         *
%
%******INPUT:                                                              *
...
[STD,SWD,satEL,MFh,MFw,Lc] = deal(SHD);%copy the contents of SHD to all the requested outputs
...
for i = 1:ncol %Loop over the Number of Receiver positions
   ...
   for j = 1:nrow %Loop over the Number of Satellite Elevations
      ...
      try%ALgorithm from Black(1978)
       ...
       catch %ALgorithm from Seeber(2003) 
           
            if Lc(j,i) == 1               
              MFh(j,i) = csc(satEL(j,i))-(1.92/((satEL(j,i).^2)+0.6));
              MFw(j,i) = csc(satEL(j,i))-(1.92/((satEL(j,i).^2)+0.6));             
            else                
                MFh(j,i) = (1-((cos(satEL(j,i)))./(1+(1-Lc(j,i)).*(Hd(i)./Re))).^2).^(-0.5)-(1.92/((satEL(j,i).^2)+0.6));%Hydrostatic Mapping Function
                MFw(j,i) = (1-((cos(satEL(j,i)))./(1+(1-Lc(j,i)).*(Hw(i)./Re))).^2).^(-0.5)-(1.92/((satEL(j,i).^2)+0.6));%Wet Mapping Function                     
