%**************************************************************************
%DESCRIPTION:                                                              * 
%           "getMETpara_GPT3" computes surface Meteorological parameters   *
%           such as pressure, temperature, temperature                     *
...
%=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-*
%USAGE:                                                                    *
%      [P,T,E,TM,lambda,dT,undu,ah,aw]=getMETpara_GPT3(Time,ReceiverPos,...
%                                                    grid,grid_res,Timevar)*
%=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
%*******EXAMPLE:                                                           +
%=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-+
...
%           OR                                                             +
%      grid = readGPTgrid('gpt3_5.mat','GPT2w',5) ;                        +

%2. NOW RUN THE "getMETpara_GPT3.m" TO COMPUTE MET PARAMETERS              +
%INPUTS(eg):                                                               +
%1.Time        = [2019 1 5]                                                +
%2.ReceiverPos = [6 40 3.7449 -1 34  0.7263  260.2410]                     +
%3.grid        = grid (i.e.extracted from "readGPTgrid.m")                 +
%4.grid_res    = 5                                                         +
%5.Timevar     = 0 i.e. 0 means GPT3 with time variation, 1 means static   +

%==> [P,T,E,TM,lambda,dT,undu,ah,aw]=getMETpara_GPT3(Time,ReceiverPos,grid,grid_res,Timevar)
%
%i.e.[P,T,E,TM,lambda,dT,undu,ah,aw]=getMETpara_GPT3([2019 1 5],[6 40 3.7449 -1 34  0.7263  260.2410],grid,5,0)
%=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-+
...
function [P,T,E,TM,lambda,dT,undu,ah,aw]=getMETpara_GPT3(Time,ReceiverPos,grid,grid_res,Timevar)
  ...
  if isequal(Identical,1) %if ROWS ARE IDENTICAL
      ...
      for i=1:nrow_time
          ...
          if grid_res == 1 %if grid resolution is 1 x 1 degree
             ...
             [p(i,1),T(i,1),dT(i,1),Tm(i,1),e(i,1),ah(i,1),aw(i,1),la(i,1),undu(i,1)] = GPT3_1_x_1(UTCtime(i,:),lat(i),lon(i),hell(i),grid,Timevar); 
          
          elseif grid_res == 5 %if grid resolution is 5 x 5 degree
              
                 %Call the the 5 degree grid version;"GPT3_5_x_5.m" 
                [p(i,1),T(i,1),dT(i,1),Tm(i,1),e(i,1),ah(i,1),aw(i,1),la(i,1),undu(i,1)] = GPT3_5_x_5(UTCtime(i,:),lat(i),lon(i),hell(i),grid,Timevar);        
          end 
      end
        ...
        for i=1:nrow_time %LOOP OVER TIME
           ...
           for j=1:nrow_pos %LOOP OVER POSITIONS
               ...
               if grid_res == 1 %if grid resolution is 1 x 1 degree
                  ...
                  [p(j,i),T(j,i),dT(j,i),Tm(j,i),e(j,i),ah(j,i),aw(j,i),la(j,i),undu(j,i)] = GPT3_1_x_1(UTCtime(i,:),lat(j),lon(j),hell(j),grid,Timevar);
           
               elseif  grid_res == 5 %if grid resolution is 5 x 5 degree
                       %Call the "GPT2w_5_x_5.m" Function 
                       [p(j,i),T(j,i),dT(j,i),Tm(j,i),e(j,i),ah(j,i),aw(j,i),la(j,i),undu(j,i)] = GPT3_5_x_5(UTCtime(i,:),lat(j),lon(j),hell(j),grid,Timevar);                  
               end          
           end 
        end 
...
%C_1.SUBROUTINE TO COMPUTE METEOROLOGICAL PARAMETERS FROM 5x5 GRID MODEL
function [p,T,dT,Tm,e,ah,aw,la,undu,Gn_h,Ge_h,Gn_w,Ge_w] = GPT3_5_x_5(UTCtime,lat,lon,h_ell,grid,Timevar)
...
%            the coefficients.
%USAGE:
%      [p,T,dT,Tm,e,ah,aw,la,undu,Gn_h,Ge_h,Gn_w,Ge_w] = GPT3_5_x_5(UTCtime,lat,lon,hell,grid,Timevar) *
%INPUTs:                                                                   *
...
%10.   Gn_h: hydrostatic north gradient in m (vector)                      *
%11.   Ge_h: hydrostatic east gradient in m (vector)                       *
%12.   Gn_w: wet north gradient in m (vector)                              *
%13    Ge_w: wet east gradient in m (vector)                               *
%=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
%NOTE:                                                                     *     
...
%C_2.******SUBROUTINE TO COMPUTE METEOROLOGICAL PARAMETERS FROM 1x1 GRID MODEL
function [p,T,dT,Tm,e,ah,aw,la,undu,Gn_h,Ge_h,Gn_w,Ge_w] = GPT3_1_x_1(UTCtime,lat,lon,h_ell,grid,Timevar)
...
%            the coefficients.
%USAGE:
%      [p,T,dT,Tm,e,ah,aw,la,undu,Gn_h,Ge_h,Gn_w,Ge_w] = GPT3_1_x_1(UTCtime,lat,lon,hell,grid,Timevar) 
%INPUTs:                                                                   *
%1.   UTCtime:  UTC time in [Year,Month,Day,Hour,Minute,Seconds]           *                                                                
%2.       lat:  Station geodetic latitude in radians [-pi/2:+pi/2] (n x 1) *
%3.       lon:  Station geodetic longitude in radians [-pi:pi] or [0:2pi]  *
