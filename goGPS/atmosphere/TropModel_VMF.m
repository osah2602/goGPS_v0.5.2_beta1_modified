...
%--------------------------------------------------------------------------
%INPUT:                                                                    *
%       The function accepts SIX(6) sets of inputs:                       *
...
%--------------------------------------------------------------------------+
%6.    SatPos   : Satellite position(s) in ECEF(XYZ) Coordinates OR        * 
...
%=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
%OUTPUTs                                                                   *
%1.      SHD : Slant Hydrostatic Delay                                     *
%2.      SWD : Slant Wet Delay                                             *
%3.      STD : Slant Total Delay                                           *           
%4.      ZHD : Zenith Hydrostatic Delay (m),valid at h                     *
%5.      ZWD : Zenith Wet Delay (m), valid at h                            *
%6.      ZTD : Zenith Total Delay (m), valid at h                          *
%7.     VMFh : Vienna Hydrostatic mapping function with provision of satELEV         
%8      VMFw : Vienna Wet mapping function with provision of of satELEV    *
%7.     VMFh : Hydrostatic mapping function coefficient(ah) valid at h     *
%               without the provision of satELEV                           *
%8.     VMFw : Wet mapping function coefficient(aw) valid at h  without    *
%               the provision of satELEV                                   *
%9.VMF_model : VMF type (VMF1 OR VMF3)                                     *
%--------------------------------------------------------------------------
%******NOTE:
%           If Satellite elevations(satELEV) are provided,then VMF factors * 
%           (VMFh,VMFw)and Slant Tropospheric Delays are computed as well. *
%           When satELEV are not provided,zeros of slant delays(STD,SHD,SWD)
%           and VMF coefficients(ah,aw) are returned. I.e.:
%           STD,SHD,SWD = 0's OF size of ZHD or ZWD or ZTD;
%           VMFh will be = ah and  VMFw will also be = aw ;
%--------------------------------------------------------------------------
%=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-                        *
% WRITTEN BY:                                                              +
...
function [STD,SHD,SWD,ZTD,ZHD,ZWD,VMFh,VMFw,VMF_model] = TropModel_VMF(Time,Lat,Lon,h,VMFgrids,SatPos)
        ...
        if (any(nargin==[6,5,4,3,2])) 
           ...
           switch ncol_lon
               ...
               if nargin == 5   %IF 5 INPUTS ARE PROVIDED
                  ...
                  if ~isstruct(VMFgrids)%Determine whether VMFgrids input is not structure array
                     
                     [nrow,ncol]   = size(VMFgrids);%GET SIZE OF VMFgrids
                     
                     %***ROUND VMFgrids TO THE NEAREST THOUSAND             
                     t1 = roundn(VMFgrids,3); 
                     
                     if any([any([(nrow > 1 & ncol == 1),(nrow == 1 & ncol > 1),all(t1(:,:))==0]),...
                             any([(ncol == 3 & (nrow > ncol | nrow < ncol | nrow == ncol)),...
                                  (nrow == 3 & (ncol > nrow | ncol < nrow | ncol == nrow))])])
                              
                             
              
                         SatPos   = VMFgrids;%ASSUMING VMFgrids POSITION TO BE SatPos 
                  ...
                  elseif isempty(VMFgrids) %Determine whether VMFgrids array is empty
            ...
            elseif  (any(nargin==[4,3,2])) %IF THREE INPUTS ARE PROVIDED
                     ...
                     if nargin == 4 %IF 4 INPUTS ARE PROVIDED
                        ...
                        %OR VMFgrids & SatPos ASSIGNED TO Lon & h POSITIONS RESPECTIVELY
                        %NOTE:
                        %     THIS HAPPENS WHEN USER DECIDE TO PROVIDE LAT,LON & h
                        %     COORD AS A SINGLE INPUT,I.E.[Lat Lon h].IN THAT CASE,
                        %     VMFgrids WILL TAKE THE POSITION OF Lon & SatPos THAT OF h
                        ...
                        if all([isstruct(Lon),any([any([isempty(h),isempty(t1)]),any([(nrow > 1 & ncol == 1),(nrow == 1 & ncol > 1),all(t1(:,:))==0]),...
                                any([(ncol == 3 & (nrow > ncol | nrow < ncol | nrow == ncol)),(nrow == 3 & (ncol > nrow | ncol < nrow | ncol == nrow))])])])
                        ...
                        elseif all([isstruct(h),any([any([isempty(Lon),isempty(t2)]),any([(nrow1 > 1 & ncol1 == 1),(nrow1 == 1 & ncol1 > 1),all(t2(:,:))==0]),...
                                    any([(ncol1 == 3 & (nrow1 > ncol1 | nrow1 < ncol1 | nrow1 == ncol1)),(nrow1 == 3 & (ncol1 > nrow1 | ncol1 < nrow1 | ncol1 == nrow1))])])])
                                
                                %ASSIGNMENT
                                VMFgrids = h; %ASSUMING h POSITION TO BE GTrop_coeff
                        ...
                        end %//if all([isstruct(Lon),any([any([isempty(h),isempty(t1)]),any([(nrow > 1 & ncol == 1),(nrow == 1 & ncol > 1),all(t1(:,:))==0]),...
                            %                        any([(ncol == 3 & (nrow > ncol | nrow < ncol | nrow == ncol)),(nrow == 3 & (ncol > nrow | ncol < nrow | ncol == nrow))])])])
                     ...
                     elseif nargin == 3 %IF 3 INPUTS ARE PROVIDED
                            ...
                            if isstruct(Lon)
                               ...
                               SatPos   = [];%Assigning empty matrix ([]) to SatPos 
                                 ...
                                 if any([any([(nrow1 > 1 & ncol1 == 1),(nrow1 == 1 & ncol1 > 1),all(t2(:,:))==0]),...
                                 ...
                                 end %//if any([any([(nrow1 > 1 & ncol1 == 1),(nrow1 == 1 & ncol1 > 1),all(t2(:,:))==0]),...
                                     %          any([(ncol1 == 3 & (nrow1 > ncol1 | nrow1 < ncol1 | nrow1 == ncol1)),...
                                     %               (nrow1 == 3 & (ncol1 > nrow1 | ncol1 < nrow1 | ncol1 == nrow1))])])
...
%***********COMPUTE SLANT TROPOSPHERIC DELAYS(STD,SHD,SWD)
if any([~exist('satpos_empty','var'),exist('satEL_deg','var'),~isempty(Satpos)])
    
   %1.COMPUTE VIENNA MAPPING FUNCTIONs(VMF)
   [VMFh,VMFw] = VMF(ah,aw,date,lat,lon,h,satEL_deg,VMF_model);

   %2.COMPUTE SLANT DELAYS
   [STD, SHD, SWD] = SlantTropDelay(ZHD,ZWD,VMFh,VMFw);
   
else 
     %MF COEFFICIENTS
     VMFh = ah;%Assign hydrostatic mf coefficients to VMFh instead
     VMFw = aw; %Assign wet mf coefficients to VMFw instead
...
%%%%%%%%%%%%%%%%%%%%%END OF SLANT DELAYS COMPUTATION %%%%%%%%%%%%%%%%%%%%%%
%=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
%=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-    
...
%B.SUBROUTINE TO COMPUTE VIENNA MAPPING FUNCTION(VMF)
function [VMFh,VMFw] = VMF(ah,aw,UTCtime,latD,lonD,hell,satELEV,MF_model)
       ...
       [nrow,ncol]=size(satELEV);
          ...
          for i=1:ncol %Loop over Station
             ...
             for j=1:nrow %Loop over Satellites Elevations
                ...
                %********IF VMF1
                if any([strncmpi(MF_model,'VMF1(1° x 1°)',12),strncmpi(MF_model,'VMF1(5° x 5°)',12),strncmpi(MF_model,'VMF1',4)])
                
                   %Call the "vmf1.m" function
                   [VMFh(j,i),VMFw(j,i)] = vmf1(ah(i),aw(i),UTCtime(i,:),latR(i),hell(i),satEL);
                
                   %********IF VMF3
                elseif any([strncmpi(MF_model,'VMF3(1° x 1°)',12),strncmpi(MF_model,'VMF3(5° x 5°)',12),strncmpi(MF_model,'VMF3',4)])
                    
                       %Call the "vmf3.m" function
                       [VMFh(j,i),VMFw(j,i)] = vmf3(ah(i),aw(i),UTCtime(i,:),latR(i),lonR(i),hell(i),satEL);
                end
         ...
         for i=1:ncol %Loop over Station
            ...
            %       below zero degree elevation is modeled.
                ...
                if any([strncmpi(MF_model,'VMF1(1° x 1°)',12),strncmpi(MF_model,'VMF1(5° x 5°)',12),strncmpi(MF_model,'VMF1',4)])
                 ...
                 %Call the "vmf1.m" function
                 [VMFh{j,i,k},VMFw{j,i,k}] = vmf1(ah(i,k),aw(i,k),UTCtime(k,:),latR(i),hell(i),satEL);
                
                 %********IF VMF3
                elseif any([strncmpi(MF_model,'VMF3(1° x 1°)',12),strncmpi(MF_model,'VMF3(5° x 5°)',12),strncmpi(MF_model,'VMF3',4)])
                    
                       %Call the "vmf3.m" function
                       [VMFh{j,i,k},VMFw{j,i,k}] = vmf3(ah(i,k),aw(i,k),UTCtime(k,:),latR(i),lonR(i),hell(i),satEL);
                end 
        ...
        for i=1:ncol %Loop over Station
           ...
           for j=1:nrow %Loop over Satellites 
             ...
             if any([strncmpi(MF_model,'VMF1(1° x 1°)',12),strncmpi(MF_model,'VMF1(5° x 5°)',12),strncmpi(MF_model,'VMF1',4)])
                ...
                %Call the "vmf1.m" function
                [VMFh(j,i),VMFw(j,i)] = vmf1(ah(i),aw(i),UTCtime,latR(i),hell(i),satEL);
         
              %********IF VMF3
             elseif any([strncmpi(MF_model,'VMF3(1° x 1°)',12),strncmpi(MF_model,'VMF3(5° x 5°)',12),strncmpi(MF_model,'VMF3',4)])
                    
                    %Call the "vmf3.m" function
                    [VMFh(j,i),VMFw(j,i)] = vmf3(ah(i),aw(i),UTCtime,latR(i),lonR(i),hell(i),satEL);
             end 
...
%B.1.SUBROUTINE TO COMPUTE VIENNA MAPPING FUNCTION1(VMF1)
...
%B.2.SUBROUTINE TO COMPUTE VIENNA MAPPING FUNCTION 3(VMF3)
...
%C.SUBROUTINE TO READ VMF1 GRID FILE & EXTRACT COEFFICIENTs(ZHD,ZWD,ZTD,ah,aw)
...
%D.SUBROUTINE TO READ VMF3 GRID FILE & EXTRACT COEFFICIENTs(zhd,zwd,ztd,ah,aw)
...
%E.SUBROUTINE TO IMPORT VMF3 OROGRAPHY FILE
...
%F.1.*********SUBROUTINE TO COMPUTE DAY OF YEAR(DoY)
...
%F.2.*********SUBROUTINE TO CONVERT TWO DIGITs TO YEAR 4 DIGITS
...
%****************SUB-ROUTINE TO COMPUTE SATELLITE ELLEVATIONS
